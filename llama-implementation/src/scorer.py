...
def evaluate_mts(df: pd.DataFrame) -> dict:
    """
    df must have columns: pred_section_header, ref_section_header,
                          pred_section_text,   ref_section_text
    """
    ...
    # Aggregate score (mirrors MEDIQA-Chat 2023 Task A): mean of rouge1, bertscore_f1, bleurt
...
def _get_section_tagger():
    ...
    if _SECTION_TAGGER is None:
        # SectionTagger lives in the ACI-Bench baselines directory
        tagger_dir = os.path.join(
            os.path.dirname(__file__), "..", "dataset", "aci", "baselines"
        )
...
def evaluate_aci(df: pd.DataFrame) -> dict:
    """
    df must have columns: pred_note, ref_note
    """
    ...
    # Aggregate score: mean rouge1, bertscore_f1, bleurt over full note
    results["aggregate_score"] = float(np.mean([
        full_metrics["rouge1"],
